...
namespace CaptainPinkTurd.AnimationSystem
{
    public abstract class AnimationControllerBase : GameObjectBase
    {
            ...
            bool playInReverse = false, bool isClamp = false, float speed = 1f)
        {
            ...
            // kill any leftover timer from a previous PlayAnimation call
            timer?.Dispose();
            if (playInReverse)
            {
                ...
                // Or shorter if crossfade still applies
                timer = animator.updateMode == AnimatorUpdateMode.UnscaledTime ? 
                    new CountdownTimer(clip.length, midEventTimer, true) : 
                    new CountdownTimer(clip.length, midEventTimer); 
            ...
            {
                var timerLength = GetAnimationClipLength(animationHash) - crossfadeTime;
                timer = animator.updateMode == AnimatorUpdateMode.UnscaledTime ? 
                    new CountdownTimer(timerLength, midEventTimer, true) : 
                    new CountdownTimer(timerLength, midEventTimer); 
                timer.OnTimerStart += () =>
                {
